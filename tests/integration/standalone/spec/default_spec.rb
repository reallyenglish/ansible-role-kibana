...
describe server(:kibana) do
  ...
  it "uploads sample logs to elasticsearch" do
    ...
    retry_and_sleep do
      ...
      result = current_server.ssh_exec(format("curl -XPOST 'localhost:9200/_bulk?pretty' -d '%s'", json))
  ...
  describe capybara("http://#{server(:kibana).server.address}:5601/status") do
    it "returns 200" do
      retry_and_sleep(tries: 10, sec: 10, verbose: true) do
        visit "/"
        raise ServiceNotReady if page.status_code.nil? || (page.status_code == 503)
      end
